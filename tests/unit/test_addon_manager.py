...
@pytest.mark.asyncio
async def test_install_addons_empty_list(manager):
    """Test installing with empty addon list."""
    result = await manager.install_addons([])
...
async def test_install_multiple_addons(mock_addon_class, manager):
    ...
    result = await manager.install_addons(["ingress", "nginx", "ingress-nginx"])
...
@pytest.mark.asyncio
async def test_install_addons_invalid_name(manager):
    """Test installing with invalid addon name."""
    result = await manager.install_addons(["invalid-addon"])
...
@pytest.mark.asyncio
@patch("agent.cluster.addons.ingress_nginx.IngressNginxAddon")
async def test_install_addons_exception(mock_addon_class, manager):
    """Test addon installation with exception."""
    mock_addon = MagicMock()
    mock_addon.run = AsyncMock(side_effect=Exception("Unexpected error"))
    mock_addon_class.return_value = mock_addon
